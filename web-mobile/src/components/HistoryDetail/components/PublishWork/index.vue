<template>
  <view :class="pageStyle['publish-content']">
    ...
    >
      ...
      <!-- 描述 -->
      <nut-form-item label="描述" prop="description" :class="pageStyle['form-item']">
        <nut-textarea
      ...
      <view :class="pageStyle['tag-group']">
        <nut-form-item prop="styleTags" :class="pageStyle['tag-selector']" label="风格标签">
          <view :class="pageStyle['tag-selector-group']">
            <view
              v-for="tag in styleTags"
              :key="tag.id"
              :class="[pageStyle['tag-selector-item'], { [pageStyle['tag-selected']]: formData.styleTags.includes(tag.name) }]"
            ...
            <AddCustomTag @save="(str) => handleAddCustomTag(TagType.STYLE, str)" />
          </view>
        </nut-form-item>
      </view>
      ...
      <!-- 场景标签 -->
      <view :class="pageStyle['tag-group']">
        <nut-form-item prop="sceneTags" :class="pageStyle['tag-selector']" label="场景标签">
          <view :class="pageStyle['tag-selector-group']">
            <view
              v-for="tag in sceneTags"
              :key="tag.id"
              :class="[pageStyle['tag-selector-item'], { [pageStyle['tag-selected']]: formData.sceneTags.includes(tag.name) }]"
            ...
            <AddCustomTag @save="(str) => handleAddCustomTag(TagType.SCENE, str)" />
          </view>
        </nut-form-item>
      </view>

      <view :class="pageStyle['btn-group']">
        <nut-button type="primary" :class="pageStyle['cancel-btn']" @click="handleCancel"> 取消 </nut-button>
        <nut-button type="primary" :class="pageStyle['submit-btn']" @click="handleSubmit"> 发布 </nut-button>
      </view>
    </nut-form>
  </view>
</template>
...
import {
  defaultSceneTags,
  defaultStyleTags, deleteCustomTagAndSave,
  formRules,
  getTagList,
...
// 处理标签点击事件
const handleTagClick = (tagName: string, tagType: 'styleTags' | 'sceneTags') => {
...
const handleSubmit = async () => {
  ...
  const response = await publishToSquare({
    ...
    drawTaskId: props.taskInfo.type === EFunctionGroupMode.DRAWING ? String(props.taskInfo.id) : undefined,
    editedTaskId:
      props.taskInfo.type !== EFunctionGroupMode.DRAWING ? String(props.taskInfo.id) : undefined,
  });
