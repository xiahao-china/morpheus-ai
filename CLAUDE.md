...
Morpheus AI 是一个 AI 创作平台，包含三个主要部分：
- **web-mobile**: 移动端 H5 前端 (Vue 3 + Vant)
- **server**: Node.js 后端服务 (Express/TypeScript)
- **ai-design-backend-main**: Java 后端服务 (Spring Boot 3.5)
...
```bash
# 安装依赖
cd web-mobile && pnpm install

# 启动开发服务器
cd web-mobile && pnpm dev

# 构建测试环境
cd web-mobile && pnpm build:test

# 构建生产环境
cd web-mobile && pnpm build:prod
```
...
# 构建 Docker 镜像
cd server && pnpm build:docker
```

### 发布命令
```bash
# 发布服务端到测试环境
pnpm run publish:server-test

# 发布移动端到测试环境
pnpm run publish:mobile-test

# 同步 Nginx 配置
pnpm run publish:nginx
```
...
### 服务端配置
- **核心配置**: 位于 `server/src/config/`，支持环境差异化
- **测试环境**: 使用根目录下的 `config.test.json`
- **开发环境**: 使用根目录下的 `config.dev.json`
